...
static const String & toTmpString(const String & s1) { return s1; }
static String toTmpString(Value & s1) {
...
static String toTmpString(double s1) {
    Value tmp(s1);
...
static String join(T1 && s1, T2 && s2) {
...
template<typename T>
static double lengthOf(T && s1) {
    return lengthOf(toTmpString(s1));
}


static String letterOf(const String & s1, uint64_t i) {
    return i > 0 && i <= s1.length ? String(s1.data[i - 1], 1, true) : String();
}
template<typename T>
static String letterOf(T && s1, uint64_t i) {
