...
#define __str_make_bool_op(op)                                                                     \
    ...
    inline bool operator op(const wchar_t * s1, String && s2) { return wcscmp(s1, s2.data) op 0; } \
    inline bool operator op(String && s1, const wchar_t * s2) { return wcscmp(s1.data, s2) op 0; } \
